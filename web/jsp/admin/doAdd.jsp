...
<%
    ...
    System.out.println("上传路径是："+uploadFilePath);
			...
			while(iter.hasNext()){
				...
				if(item.isFormField()){// 如果是普通表单控件 
					...
					if(fieldName.equals("title")){
						newsDetails.setTitle(item.getString("UTF-8"));//获得该字段值
					}else if(fieldName.equals("categoryId")){
						newsDetails.setCategoryId(Integer.parseInt(item.getString()));
					}else if(fieldName.equals("summary")){
						newsDetails.setSummary(item.getString("UTF-8"));
					}else if(fieldName.equals("newscontent")){
						newsDetails.setContent(item.getString("UTF-8"));
					}else if(fieldName.equals("author")){
						newsDetails.setAuthor(item.getString("UTF-8"));
					}
				}else{// 如果为文件域
					...
					if(fileName != null && !fileName.equals("")){
						...
						newsDetails.setPicPath(uploadFileName);
						bUpload = true;
					}
				}
			}
	...
	System.out.println("上传成功之后的文件名称：" + newsDetails.getPicPath());
    System.out.println(newsDetails.getId()+newsDetails.getContent()+newsDetails.getSummary());

    //调用后台的方法，将新闻信息插入数据库中
	bRet = newsService.addNews(newsDetails);
	if(bRet)
